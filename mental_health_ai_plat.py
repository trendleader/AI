...
class OpenAIAssistant:
    ...
    def analyze_data(self, data, query, agent_type):
        ...
        try:
            # Prepare detailed data summary for analysis
            data_summary = self._prepare_data_summary(data, agent_type)
            
            # Create agent-specific prompts
            system_prompt = self._get_system_prompt(agent_type)
            user_prompt = f"""
            I have completed a mental health analytics query: "{query}"
            
            Here is the ACTUAL DATA from our SQL Server database [dbo].[vw_MentalHealthRAG]:
            
            {data_summary}
            
            This is REAL healthcare data from Transforming Lives Mental Health Counseling.
            
            Please analyze this specific data and provide:
            1. **Key Data Insights**: What does this actual data tell us?
            2. **Trends & Patterns**: What patterns do you see in the numbers?
            3. **Business Recommendations**: Specific actions based on THIS data
            4. **Risk Factors**: Any concerns or red flags in the data
            5. **Performance Metrics**: How is the organization performing based on these numbers?
            
            Focus specifically on the {agent_type.lower()} aspects. Use the actual numbers and values provided.
    ...
    def _prepare_data_summary(self, data, agent_type):
        ...
        # Add column names
        summary += f"Available columns: {', '.join(data.columns[:10])}{'...' if len(data.columns) > 10 else ''}\n\n"
        ...
        # Agent-specific data summaries with more detail
        if agent_type == "Billing":
            billing_cols = ['billed_amount', 'paid_amount', 'patient_responsibility', 'copay_amount', 'total_billed', 'total_paid', 'payment_percentage']
            available_cols = [col for col in billing_cols if col in data.columns]
            if available_cols:
                summary += "Financial Metrics Analysis:\n"
                for col in available_cols:
                    if data[col].dtype in ['int64', 'float64'] and data[col].notna().sum() > 0:
                        summary += f"- {col}: Mean=${data[col].mean():.2f}, Total=${data[col].sum():.2f}, Min=${data[col].min():.2f}, Max=${data[col].max():.2f}\n"
                
                # Add categorical analysis
                categorical_cols = ['visit_month_name', 'visit_year', 'cost_category', 'plan_type']
                for col in categorical_cols:
                    if col in data.columns and data[col].notna().sum() > 0:
                        counts = data[col].value_counts().head(5).to_dict()
                        summary += f"- {col} distribution: {counts}\n"
        
        elif agent_type == "Scheduling":
            summary += "Scheduling Metrics Analysis:\n"
            if 'visit_date' in data.columns and data['visit_date'].notna().sum() > 0:
                summary += f"- Date range: {data['visit_date'].min()} to {data['visit_date'].max()}\n"
            if 'session_length_minutes' in data.columns and data['session_length_minutes'].notna().sum() > 0:
                summary += f"- Session length: Mean={data['session_length_minutes'].mean():.1f} min, Min={data['session_length_minutes'].min()}, Max={data['session_length_minutes'].max()}\n"
            
            # Scheduling-specific categories
            sched_cols = ['visit_day_of_week', 'visit_type', 'provider_full_name']
            ...
            summary += "Clinical Research Analysis:\n"
            research_cols = ['diagnosis_code', 'condition_category', 'provider_specialty', 'age_group', 'gender']
            for col in research_cols:
                if col in data.columns and data[col].notna().sum() > 0:
                    counts = data[col].value_counts().head(5).to_dict()
                    summary += f"- {col}: {counts}\n"
            
            # Numeric research metrics
            numeric_research = ['member_age', 'visit_sequence_number']
            for col in numeric_research:
                if col in data.columns and data[col].notna().sum() > 0:
                    summary += f"- {col}: Mean={data[col].mean():.1f}, Range={data[col].min()}-{data[col].max()}\n"
        
        return summary[:3000]  # Increased limit for more detail
...
CONFIDENTIAL - For Internal Use Only"""
        ...
        for i, analysis in enumerate(analyses):
            ...
            if analysis.get('ai_insights'):
                ...
                # Truncate AI insights for slide
                ai_text = analysis['ai_insights'][:200] + "..." if len(analysis['ai_insights']) > 200 else analysis['ai_insights']
                p = tf.add_paragraph()
                p.text = ai_text
                p.level = 2
...
if st.sidebar.button("🗑️ Clear All Analyses", help="Clear all completed analyses from all agents"):
    for agent in st.session_state.enhanced_agents.values():
        agent.analyses.clear()
